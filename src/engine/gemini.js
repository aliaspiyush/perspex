// Gemini API Client for LLM-powered candidate ranking
...
export async function parseJobDescription(apiKey, jdText) {
...
Return ONLY a JSON object with this exact structure:
{
  ...
  "required_skills": [
    { "name": "string", "importance": "critical|high|medium", "context": "why this skill matters for the role" }
  ],
  "preferred_skills": [{ "name": "string", "importance": "medium|low" }],
  "semantic_clusters": ["cluster1", "cluster2"],
  "domain_context": "string describing the technical domain deeply",
...
export async function scoreCandidateBatch(apiKey, parsedJD, candidates) {
...
Skills: ${skills}
Career: ${career}
Education: ${(c.education || []).map(e => `${e.degree} from ${e.institution} (${e.tier || 'N/A'})`).join('; ')}
Signals: open_to_work=${sigs.open_to_work_flag}, response_rate=${sigs.recruiter_response_rate}, interview_rate=${sigs.interview_completion_rate}, github_score=${sigs.github_activity_score}, notice_days=${sigs.notice_period_days}
...
Return ONLY a JSON array with exactly ${candidates.length} objects in the same order as the candidates:
[
  {
    "candidate_id": "string",
    ...
    "reasoning": "2-3 sentence explanation of WHY this candidate ranks here — be specific about what signals drove the score",
    "key_strengths": ["strength1", "strength2"],
    "concerns": ["concern1"] or [],
    "is_honeypot": boolean (true if skill claims are inconsistent with experience)
  }
]`;
...
// ── PHASE 3: Generate final ranking summary ────────────────────────────────
export async function generateRankingSummary(apiKey, parsedJD, topCandidates) {
  const top5 = topCandidates.slice(0, 5);
  const prompt = `You are a senior technical recruiter. Write a concise executive summary (3-4 sentences) of the top ${top5.length} candidates found for the role of "${parsedJD.role_title}".

Top candidates:
${top5.map((c, i) => `#${i + 1}: ${c.candidate_id} — Score: ${c.overall_score}/100 — ${c.reasoning}`).join('\n')}

Write a brief executive summary explaining the quality of this shortlist and what made the top candidates stand out. Be specific and analytical.`;
