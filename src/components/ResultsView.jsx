...
export default function ResultsView({ result, onReset }) {
  ...
  const renderSidebarContent = (candidate) => (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex flex-col p-4 gap-4 border-b border-[var(--border)] shrink-0">
        ...
        <div className="flex items-center justify-between text-[14px] tabular-nums">
          ...
          <span className="font-[600] text-[var(--text-primary)]">{candidate.overall_score}</span>
          <Bar value={parseFloat(candidate.overall_score || 0) / 100} />
        </div>
        
        <div className="flex items-center justify-between text-[14px] tabular-nums">
          <span className="text-[var(--text-muted)] w-24">Semantic</span>
          <span className="text-[var(--text-primary)]">{candidate.semantic_fit}</span>
          <Bar value={parseFloat(candidate.semantic_fit || 0) / 100} />
        </div>
        
        <div className="flex items-center justify-between text-[14px] tabular-nums">
          <span className="text-[var(--text-muted)] w-24">Behavioral</span>
          <span className="text-[var(--text-primary)]">{candidate.signal_quality}</span>
          <Bar value={parseFloat(candidate.signal_quality || 0) / 100} />
        </div>
        
        <div className="flex items-center justify-between text-[14px] tabular-nums">
          <span className="text-[var(--text-muted)] w-24">Experience</span>
          <span className="text-[var(--text-primary)]">{candidate.career_relevance}</span>
          <Bar value={parseFloat(candidate.career_relevance || 0) / 100} />
        </div>
      </div>
      ...
      <div className="flex flex-col p-4 gap-4 border-b border-[var(--border)] shrink-0">
        ...
        <div className="grid grid-cols-2 gap-y-4 gap-x-2">
          <div className="flex flex-col">
            ...
            <span className="text-[14px] font-[500] text-[var(--text-primary)]">{candidate.redrob_signals?.open_to_work_flag ? 'Yes' : 'No'}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[11px] text-[var(--text-faint)]">Notice Period</span>
            <span className="text-[14px] font-[500] text-[var(--text-primary)]">{candidate.redrob_signals?.notice_period_days || 0}d</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[11px] text-[var(--text-faint)]">Response Rate</span>
            <span className="text-[14px] font-[500] text-[var(--text-primary)]">{Math.round((candidate.redrob_signals?.recruiter_response_rate || 0)*100)}%</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[11px] text-[var(--text-faint)]">GitHub Score</span>
            <span className="text-[14px] font-[500] text-[var(--text-primary)]">{candidate.redrob_signals?.github_activity_score?.toFixed(1) || '0.0'}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[11px] text-[var(--text-faint)]">Work Mode</span>
            <span className="text-[14px] font-[500] text-[var(--text-primary)]">{candidate.profile?.work_mode || 'Hybrid'}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[11px] text-[var(--text-faint)]">Location</span>
            <span className="text-[14px] font-[500] text-[var(--text-primary)] truncate max-w-full" title={candidate.profile?.location || 'Unknown'}>{candidate.profile?.location || 'Unknown'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col p-4 gap-2">
        ...
        <div className="text-[13px] text-[var(--text-muted)] leading-relaxed">
          <ul className="list-disc pl-4 space-y-1">
            {(candidate.key_strengths || []).slice(0, 3).map((s, idx) => (
              <li key={idx}>{s}</li>
            ))}
            {(!candidate.key_strengths || candidate.key_strengths.length === 0) && (
              <li>{candidate.reasoning || 'Strong overall match with role requirements.'}</li>
            )}
          </ul>
        </div>
      </div>
    ...
    <div className="h-full flex flex-col relative w-full">
      ...
      <div className="shrink-0 py-4 px-6 border-b border-[var(--border)] flex items-center justify-between bg-[var(--bg)]">
        ...
        <div className="flex items-center gap-[8px]">
          ...
          <button onClick={handleExport} className="h-8 px-4 flex items-center justify-center text-[14px] font-[500] transition-colors border border-[var(--border)] bg-transparent text-[var(--text-primary)] hover:bg-[var(--surface)]">
            ...
            <thead className="sticky top-0 bg-[var(--bg)] z-10 border-b border-[var(--border)]">
              <tr>
                <th className="px-4 py-2 text-[12px] font-[600] uppercase tracking-[0.06em] text-[var(--text-muted)]">
                  <div className="flex items-center gap-1">Rank <ChevronUp size={14} strokeWidth={1.5} /></div>
                </th>
      ...
      <div className={`hidden md:flex fixed top-0 right-0 h-full w-[340px] bg-[var(--bg)] border-l border-[var(--border)] z-30 flex-col transition-transform duration-200 ease-out ${
        selectedCandidate ? 'translate-x-0' : 'translate-x-full'
      }`}>
        {selectedCandidate && renderSidebarContent(selectedCandidate)}
      </div>

      {/* Mobile backdrop */}
      {selectedCandidate && (
        <div className="md:hidden fixed inset-0 bg-black/20 z-20" onClick={() => setSelectedCandidate(null)} />
      )}

      {/* Mobile Bottom Sheet */}
      <div className={`md:hidden fixed bottom-0 left-0 right-0 z-30 bg-[var(--bg)] border-t border-[var(--border)] rounded-t-[12px] transition-transform duration-200 ease-out ${
